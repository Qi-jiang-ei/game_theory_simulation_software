...
export type SimulationResult = {
  step: number;
  playerChoices: { [playerId: number]: string };
  payoffs: { [playerId: number]: number };
  beliefs?: { [playerId: number]: { [strategy: string]: number } };
...
export const CLASSIC_MODELS: GameModel[] = [
  ...
  },
  {
    ...
    description: "一个市场中的两个企业，领导者先决定产量，跟随者观察后决定自己的产量。这是一个经典的产业组织理论模型，展示了市场中先行者优势。领导者通过先决策来影响跟随者的选择，从而获得更大的市场份额和利润。这个模型在寡头市场竞争分析中有重要应用。",
    players: [
